...
class FurnitureBotCore:
    ...
    async def handle_text_message(self, platform: Platform, user_id: int, text: str):
        ...
        # Если ожидаем телефон и пользователь ввел вручную
            ...
            phone = text.strip()
            if (phone.startswith('+7') and len(phone) == 12) or \
               ...
               (phone.startswith('7') and len(phone) == 11):
                ...
                await self.send_message(
                    ...
                    "Или используйте кнопку для отправки номера"
                )
...
class TelegramAdapter:
    ...
    async def edit_message(self, user_id: int, message_id: int, text: str, keyboard=None):
        await self.application.bot.edit_message_text(
            chat_id=user_id, message_id=message_id, text=text, reply_markup=keyboard, parse_mode='Markdown'
        )
